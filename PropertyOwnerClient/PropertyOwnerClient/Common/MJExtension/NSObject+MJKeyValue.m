...
/**
 *  通过字典来创建一个模型  (主类下有多个，取一个)
 *  @param keyValues 字典
 *  @return 新建的对象
 */
+ (instancetype)objectWithKeyValuesOneOfMany:(NSDictionary *)keyValues  className:(NSString*)classNameOneOfMany
{
    if (![keyValues isKindOfClass:[NSDictionary class]]) {
        [NSException raise:@"keyValues is not a NSDictionary" format:nil];
    }
    ...
    id model = [[self alloc] init];
    [model setKeyValuesOneOfMany:keyValues classForName:classNameOneOfMany];
    return model;
}
...
- (void)setKeyValuesOneOfMany:(NSDictionary *)keyValues classForName:(NSString *)classNameOneOfMany
{
    if (![keyValues isKindOfClass:[NSDictionary class]]) {
        [NSException raise:@"keyValues is not a NSDictionary" format:nil];
    }
    ...
    [self enumerateIvarsWithBlock:^(MJIvar *ivar, BOOL *stop) {
        ...
        // 2.如果是模型属性  - (NSDictionary *)objectClassInArray:(NSString*) className
        if (ivar.type.typeClass && !ivar.type.fromFoundation) {
            value = [ivar.type.typeClass objectWithKeyValues:value];
        } else if ([self respondsToSelector:@selector(objectClassInArrayOneOfMany:)]){
            
            Class objectClass = [self objectClassInArrayOneOfMany:classNameOneOfMany] [ivar.propertyName];
          //  NSLog(@"%d",[value count]);
            if (objectClass&&[value objectForKey:@"records"]) {
                value =[objectClass objectArrayWithKeyValuesArray:[value objectForKey:@"records"]];
            }else if(objectClass&&![value objectForKey:@"records"]){
                value =[objectClass objectArrayWithKeyValuesArray:value];
            }
