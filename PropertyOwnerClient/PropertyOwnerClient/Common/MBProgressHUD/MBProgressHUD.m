...
@property (atomic, MB_STRONG) HZActivityIndicatorView *indicator;
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.buttonTitle = nil; //add
        self.buttonFont = [UIFont boldSystemFontOfSize:kLabelFontSize]; //add
		self.opacity = 0.8f;
        self.color = nil;
...
- (void)dealloc {
	...
	[indicator release];
	[label release];
    cancelButton = nil; //add
    [buttonTitle release];  //add
...
//	detailsLabel.textColor = [UIColor whiteColor];
    detailsLabel.textColor = DEFAULT_COLOR;
...
- (void)setupCancelButton {
    ...
    [cancelButton setTitle:self.buttonTitle forState:UIControlStateNormal];
...
- (void)cancelButtonPressed:(id)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(hudCancelUploadAction:)])
        [self.delegate hudCancelUploadAction:nil];
}
...
//										 initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge]);
        self.indicator = MB_AUTORELEASE([[HZActivityIndicatorView alloc] initWithFrame:self.bounds]);
        self.indicator.backgroundColor = self.backgroundColor;
        self.indicator.opaque = YES;
        self.indicator.steps = 8;
        self.indicator.finSize = CGSizeMake(8, 8);
        self.indicator.indicatorRadius = 10;
        self.indicator.stepDuration = 0.100;
        self.indicator.color = DEFAULT_COLOR;
        self.indicator.cornerRadii = CGSizeMake(10, 10);
...
- (void)layoutSubviews {
    ...
    /********************** add ************************/
    CGSize buttonSize = MB_TEXTSIZE(self.buttonTitle, self.buttonFont);
    buttonSize.width = MIN(buttonSize.width, maxWidth);
    buttonSize.height = MAX(buttonSize.height, 44.);
    if (buttonSize.width > 0) {
        buttonSize = CGSizeMake(100., 36.);
    }
    /***************************************************/
...
//			@"detailsLabelText", @"detailsLabelFont", @"progress", nil];
            return [NSArray arrayWithObjects:@"mode", @"customView", @"labelText", @"labelFont",
                    @"detailsLabelText", @"detailsLabelFont", @"progress", @"buttonTitle", @"buttonFont",nil];
}
...
//		_backgroundTintColor = [[UIColor alloc] initWithWhite:1.f alpha:.1f];
        _progressTintColor = DEFAULT_COLOR;
        _backgroundTintColor = DEFAULT_COLOR;
		[self registerForKVO];
	}
	return self;
}
